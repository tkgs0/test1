...
@event_preprocessor
async def blacklist_processor(bot: Bot, event: Event):
    ...
    if (gid := str(vars(event).get('group_id', None))) and gid in blacklist['grouplist']:
        if random.random() < 0.05 and bot.self_id in [
            at.data['qq'] for at in vars(event).get('original_message', Message())['at']
        ]:
            await asyncio.sleep(random.random()+1)
            await bot.send(event, randtxt())
...
def randtxt():
    return random.choice([
        MessageSegment.image("https://github.com/tkgs0/test1/raw/main/%E9%97%AE%E5%8F%B7.jpg"),
        MessageSegment.image("https://github.com/tkgs0/test1/raw/main/%E9%98%B3%E7%97%BF%E6%80%8E%E4%B9%88%E6%B2%BB.jpg"),
        'ʕ  •ᴥ•ʔ ?',
        'ʕ  •ᴥ•ʔ……'
    ])
